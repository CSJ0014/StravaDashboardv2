...
export default function RideDetails({ activity, streams }) {
  ...
  // --- zones ---
  const powerZones = [0.56, 0.76, 0.90, 1.05, 1.20];
  const hrZones = [0.60, 0.70, 0.80, 0.90];
  const labelsP = ["Z1", "Z2", "Z3", "Z4", "Z5", "Z6"];
  const labelsH = ["Z1", "Z2", "Z3", "Z4", "Z5"];
  const powerPct = useMemo(() => calcZones(watts, powerZones, labelsP, 222), [watts]);
  const hrPct = useMemo(() => calcZones(hr, hrZones, labelsH, 200), [hr]);
